...
func Test_MYSQL_Get(t *testing.T) {
	...
	result, _, err := testStore.Get(key)
	require.NoError(t, err)
...
func Test_MYSQL_Get_NotExist(t *testing.T) {
	result, _, err := testStore.Get("notexist")
	require.NoError(t, err)
	require.Zero(t, len(result))
}
...
func Test_MYSQL_Reset(t *testing.T) {
	...
	result, _, err := testStore.Get("john1")
	require.NoError(t, err)
	require.Zero(t, len(result))

	result, _, err = testStore.Get("john2")
	require.NoError(t, err)
	require.Zero(t, len(result))
}
...
func Test_MYSQL_Non_UTF8(t *testing.T) {
	...
	result, _, err := testStore.Get("0xF6")
	require.NoError(t, err)
	require.Equal(t, val, result)
}
